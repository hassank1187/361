...
int main(int argc, char** argv) {
    ...
    double RTT = (double)((seconds * 1000000) + end.tv_usec - start.tv_usec);
    ...
    for(int i = 0; i < frag_count; i++){
        ...
        int n = 0;
        do{
            n = recvtimeout(sockfd,buf,MAXBUFLEN - 1,8*to);
            if(n == -1){
                perror("recvtimeout");
                exit(1);
            }
            if(n== -2){
                ...
                printf("retransmit occured\n");
            }
        }while(n == -2);
        gettimeofday(&end,NULL);
        seconds = (end.tv_sec - start.tv_sec);
        int prev_RTT = RTT;
        RTT = (double)((seconds * 1000000) + end.tv_usec - start.tv_usec);
        to = RTT + 4*abs(RTT - prev_RTT);
